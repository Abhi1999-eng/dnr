...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return createPageMetadata({
    title: 'Industrial Machinery Products | Die Casting, CNC & Foundry Machines',
    description:
      products?.[0]?.shortDescription ||
      'Browse industrial machinery from DNR Techno Services covering die casting machines, CNC systems, foundry equipment, automation machinery, and plant support solutions.',
    path: '/products',
...
export default async function ProductsPage() {
  ...
  const structuredData = [
    ...
    buildCollectionPageJsonLd({
      ...
      description: 'Industrial machinery and foundry, die casting, CNC, and automation equipment supplied and supported by DNR Techno Services.',
      path: '/products',
      image: resolveProductImage(products?.[0]),
    }),
    buildItemListJsonLd({
      name: 'DNR Product Range',
      path: '/products',
      items: (products || []).slice(0, 24).map((product: any) => ({
        name: product.title,
      ...
      <div className="container-wide space-y-8 py-12 md:py-14">
        <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(17,27,36,0.94),rgba(10,16,20,0.98))] p-6 shadow-[0_20px_48px_rgba(0,0,0,0.26)] md:p-8">
          <div className="space-y-3">
            ...
            <h1 className="text-4xl font-semibold text-white md:text-5xl">Industrial machinery for casting, machining, and plant support</h1>
            <p className="max-w-3xl text-slate-300">
              Explore DNR Techno Services machinery for pressure die casting, gravity die casting, squeeze casting, CNC machining, foundry support, automation, marking, and production-line execution. Each product page is built to help manufacturing teams compare machine fit, support scope, and next-step enquiry options.
            </p>
          </div>
        </div>
